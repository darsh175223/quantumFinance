...
const CoveredCall = () => {
  ...
  const sendCoveredCall = async () => {
    try {
      const response = await axios.patch('http://localhost:8080/addBearPutSpread',null, {
        params: {
          username: username,  // replace 'yourUsername' with the actual username
          symbol: selectedCompany,
          strikePrice:  stockPrice,
          lowerStrikePrice: strikePrice,  
          time: time, 
        }
      });
  ...
  return (
    <div style={{display:'flex'}}>
                    <div>
                    <h1 style={{marginRight:'-500px'}}>Bear Put Spread Strategy</h1>
                    <div style={{backgroundColor:'#22272e'}}>
                    <h5 style={{position:'absolute', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.5)',left:'40px', borderRadius:'15px',  top:'325px', color:'#c5d1de', backgroundColor:'#22272e', padding:'15px', width:'650px'}}>Profit: (Strike Price of Bought Put - Strike Price of Sold Put) - (Current Price - Strike Price of Bought Put) - Net Premium Paid</h5>
                ...
                <div>
                <h4>Enter strike price of bought Put</h4>
                <input 
                    type="number" 
                    placeholder="Bought Put Price..." 
                    value={stockPrice} 
                    style={{ borderColor: 'black'}}
                    onChange={(e) => setStockPrice(e.target.value)} 
                />
                        <h4>Enter strike price of sold Put (Make it lower than the bought put)</h4>

                <input 
                    type="number" 
                    placeholder="Sold Put Price..." 
                    value={strikePrice} 
                    style={{ borderColor: 'black'}}
