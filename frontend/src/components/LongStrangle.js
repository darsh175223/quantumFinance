...
const CoveredCall = () => {
  ...
  const sendCoveredCall = async () => {
    try {
      const response = await axios.patch('http://localhost:8080/addLongStrangle',null, {
        params: {
  ...
  return (
    <div style={{display:'flex'}}>
                    <div>
                    <h1 style={{marginRight:'-500px'}}>Long Strangle Strategy</h1>
                    <div style={{backgroundColor:'#22272e'}}>
                    <h5 style={{position:'absolute', boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.5)',left:'40px', borderRadius:'15px',  top:'320px', color:'#c5d1de', backgroundColor:'#22272e', padding:'15px', width:'650px'}}>Profit: (Current Price - Strike Price of In-the-Money Option) * Position Size - Net Premium Paid </h5>
