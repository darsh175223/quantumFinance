...
const CoveredCall = () => {
  ...
  const sendCoveredCall = async () => {
    try {
      const response = await axios.patch('http://localhost:8080/addLongCallButterflySpread',null, {
        params: {
          username: username,  // replace 'yourUsername' with the actual username
          symbol: selectedCompany,
          doubleCall:  stockPrice,
          boughtCall: strikePrice,  
          time: time, 
        }
      });
